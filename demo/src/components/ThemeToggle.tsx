...
/** Copied from production's ThemeToggle.tsx (same inline SVGs, same
 * "no icon library" convention) with only the store swapped for the
 * demo's own. */
export function ThemeToggle() {
  const theme = useDemoThemeStore((s) => s.theme);
  const toggleTheme = useDemoThemeStore((s) => s.toggleTheme);

 ...
 * per page) side by side, with per-result "Reveal contact" (enrichment,
 * 1-9 credits) and "Add to table" (free, maps into the active table's
 * existing Company/Contact/Phone/Link columns — see
 * utils/addApolloToTable.ts). All state lives in useSearchStore, not
 * component state, so it survives this tab being hidden (App.tsx's
 * tab-panel visibility toggle) the same way Table/Calendar/Calls' own
 * state does — a filter you built up and results you were reviewing
 * don't reset just from tabbing over to check something in the table. */
    ...
    <div className="search-view">
      ...
      {total > 0 && (
        <div className="search-results-header">
          <span>
            {total.toLocaleString('en-US')} result{total === 1 ? '' : 's'} — page {page} of {totalPages.toLocaleString('en-US')}
          </span>
          <div className="search-pagination">
            <button
              type="button"
              disabled={page <= 1 || peopleLoading || companiesLoading}
            ...
            <button
              type="button"
              disabled={page >= totalPages || peopleLoading || companiesLoading}
              onClick={() => void (mode === 'people' ? runPeopleSearch(page + 1) : runCompanySearch(page + 1))}
            >
              Next →
            </button>
      ...
      <div className="search-results-list">
        {mode === 'people'
          ...
          : companies.map((c) => <CompanyResultRow key={c.id} company={c} />)}
        {mode === 'people' && !peopleLoading && people.length === 0 && total === 0 && (
          <div className="empty-state">Set some filters above and search to see people here.</div>
        )}
        {mode === 'companies' && !companiesLoading && companies.length === 0 && total === 0 && (
          <div className="empty-state">Set some filters above and search to see companies here.</div>
        )}
      </div>
    </div>
  );
